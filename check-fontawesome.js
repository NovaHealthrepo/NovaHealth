...
function findHtmlFiles(dir, fileList = []) {
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            if (!file.startsWith('.') && file !== 'node_modules' && file !== 'fonts' && file !== 'images') {
    ...
    while ((match = classRegex.exec(htmlContent)) !== null) {
        ...
        while ((iconMatch = faIconRegex.exec(classValue)) !== null) {
            ...
            const excludePatterns = [
                ...
                'fa-fw', 'fa-lg', 'fa-2x', 'fa-3x', 'fa-4x', 'fa-5x', 'fa-6x', 'fa-7x', 'fa-8x', 'fa-9x', 'fa-10x',
                'fa-xs', 'fa-sm', 'fa-xl', 'fa-2xl',
...
function checkAllHtmlFiles() {
    ...
    for (const htmlPath of htmlFiles) {
        ...
        if (missingIcons.length === 0) {
            ...
            console.log(`     CSS: ${cssFiles.map(f => path.relative(BASE_DIR, f)).join(', ')}`);
            console.log(`     Icons used: ${usedIcons.size}, All defined: ✓\n`);
            totalOk++;
        } else {
            console.log(`[MISSING] ${relativePath}`);
            console.log(`     CSS: ${cssFiles.map(f => path.relative(BASE_DIR, f)).join(', ')}`);
            console.log(`     Icons used: ${usedIcons.size}, Missing: ${missingIcons.length}`);
    ...
    if (allMissingIcons.size > 0) {
        ...
        for (const icon of sortedIcons) {
            ...
            console.log(`  .${icon}::before { content: "\\????"; }  /* Used in ${files.length} file(s) */`);
        }
